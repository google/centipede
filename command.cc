...
// Returns the most suitable stdout or stderr redirection path:
// - Always prefers an explicitly provided `stdout_or_stderr`.
...
// process, if they remain unredirected (and therefore shared).
...
std::string_view ResolveOutOrErrPath(std::string_view cmd_path,
                                     std::string_view stdout_or_stderr) {
  if (!stdout_or_stderr.empty()) return stdout_or_stderr;
