...
echo -n "$pid" > "$pid_f"
  ...
  if (exit_code != EXIT_SUCCESS) {
    LOG(ERROR) << "Fork server command failed to parse or run; will proceed "
                  "without fork server";
    LogRedirectedStdoutAndStderr();
    return false;
  }

  // Open the read/write pipes for communication with the fork server. Using
  // non-blocking I/O is good and safe: 1) This prevents the `open()` calls from
  // hanging when the fork server fails to open the pipes on its side (note
  // the use of O_RDWR, not O_WRONLY, to avoid ENXIO). 2) In `Command::Execute`,
  // we wait for the return channel pipe with a `poll()`, so it should always
  // have data when we attempt to `read()` from it. See more at
  ...
  // The fork server started and the comms pipes got opened successfully. Read
  // the fork server's PID and the initial /proc/<PID>/exe symlink pointing at
  // the fork server's binary, written to the provided files by `command`.
  // `Execute()` uses these to monitor the fork server health.
  std::string pid_str;
  ReadFromLocalFile(pid_file_path, pid_str);
  CHECK(absl::SimpleAtoi(pid_str, &fork_server_pid_));
...
absl::Status Command::AssertForkServerIsHealthy() {
  ...
  // ...and it is a process with a matching binary, so it's practically
  ...
  if (stat(proc_exe.c_str(), &proc_exe_stat) != EXIT_SUCCESS) {
    return absl::UnknownError(
        absl::StrCat("Failed to access fork server's /proc/<PID>/exe, PID=",
                     fork_server_pid_));
  }
  if (proc_exe_stat.st_dev != fork_server_exe_stat_.st_dev ||
      proc_exe_stat.st_ino != fork_server_exe_stat_.st_ino) {
    return absl::UnknownError(absl::StrCat(
        "Fork server's /proc/<PID>/exe changed (new process?), PID=",
        fork_server_pid_));
  }
...
std::string Command::ReadRedirectedStdout() const {
  ...
  absl::StripAsciiWhitespace(&ret);
  return ret;
}

std::string Command::ReadRedirectedStderr() const {
  std::string ret;
  if (!err_.empty() && err_ != "2>&1" && err_ == out_)
