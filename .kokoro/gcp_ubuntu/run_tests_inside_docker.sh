...
source ./install_dependencies_debian.sh
...
bazel test "${BAZEL_ARGS[@]}" --local_test_jobs=1 :all --test_filter="-CommandDeathTest.Execute" &&
bazel test "${BAZEL_ARGS[@]}" testing:instrumentation_test testing:runner_test &&
bazel test "${BAZEL_ARGS[@]}" puzzles:all
declare -ri exit_code=$?
set -e

# Capture the build log as a fake test to reduce download spam
declare -r FULL_BUILD_LOG_FILE="bazel_full_build_log"
...
cat >"${KOKORO_ARTIFACTS_DIR}/${FULL_BUILD_LOG_FILE}/sponge_log.xml" <<DOC
