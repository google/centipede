...
# This sh_test runs various tests on test_fuzz_target_sancov,
...
target="${TEST_SRCDIR}/google3/third_party/centipede/testing/test_fuzz_target_sancov"
...
check_features_files_f1_and_f02() {
  ...
  # and so the coverage features for the loop in test_fuzz_target_sancov
  # will be different.
...
":use_pc_features:use_cmp_features:use_dataflow_features:use_path_features:"
...
# test_fuzz_target_sancov prints its input bytes in hex form,
...
echo ======== Check timeout
CENTIPEDE_RUNNER_FLAGS=":timeout_in_seconds=567:" "${target}" \
