...
LLVM_SYMBOLIZER_DIR="$(dirname $(which llvm-symbolizer))"

# Main Centipede binary.
CENTIPEDE_BINARY="${CENTIPEDE_TEST_SRCDIR}/centipede_main"
# The following variables can be overridden externally:
# Some real-life existing fuzz target prepackaged to be used with Centipede.
: "${EXAMPLE_TARGET_BINARY:=${CENTIPEDE_TEST_SRCDIR}/testing/target_example}"
# Centipede-specific test fuzz targets.
: "${TEST_TARGET_BINARY:=${CENTIPEDE_TEST_SRCDIR}/testing/test_fuzz_target_sancov}"
: "${ABORT_TEST_TARGET_BINARY:=${CENTIPEDE_TEST_SRCDIR}/testing/abort_fuzz_target_sancov}"
# llvm-symbolizer binary.
: "${LLVM_SYMBOLIZER:=${LLVM_SYMBOLIZER_DIR}/llvm-symbolizer}"

function verify_executable() {
  local executable_var_name="$1"
  local -n executable_path="$1"
  if ! [[ -x "${executable_path}" ]]; then
    echo "FAILURE: File pointed at by ${executable_var_name} does not exist or \
is not executable: ${executable_path}"
    exit 1
  fi
}

verify_executable CENTIPEDE_BINARY
verify_executable EXAMPLE_TARGET_BINARY
verify_executable TEST_TARGET_BINARY
verify_executable ABORT_TEST_TARGET_BINARY
verify_executable LLVM_SYMBOLIZER

# Shorthand for centipede --binary=target_example.
function example_fuzz() {
  "${CENTIPEDE_BINARY}" --binary="${EXAMPLE_TARGET_BINARY}" "$@"
}
# Shorthand for centipede --binary=test_fuzz_target_sancov
function test_fuzz() {
  "${CENTIPEDE_BINARY}" --binary="${TEST_TARGET_BINARY}" "$@"
}
# Shorthand for centipede --binary=abort_fuzz_target_sancov
function abort_test_fuzz() {
