...
class MockFactory : public CentipedeCallbacksFactory {
 public:
  MockFactory(CentipedeCallbacks &cb) : cb_(cb) {}
  CentipedeCallbacks *create(const Environment &env) override { return &cb_; }
  void destroy(CentipedeCallbacks *cb) { EXPECT_EQ(cb, &cb_); }
...
TEST(Centipede, InputFilter) {
  ...
  env.log_level = 0;   // Disable most of the logging in the test.
  // Add %f so that we don't depend on the fork_server_helper_path.
  env.input_filter =
      "%f" + devtools_build::GetDataDependencyFilepath(
                 "google3/third_party/centipede/testing/test_input_filter");
  CentipedeMock mock(env);
  MockFactory factory(mock);
...
class MutateCallbacks : public CentipedeCallbacks {
 public:
  MutateCallbacks(const Environment &env) : CentipedeCallbacks(env) {}
  // Will not be called.
  bool Execute(std::string_view binary, const std::vector<ByteArray> &inputs,
               BatchResult &batch_result) override {
    CHECK(false);
  ...
  // Will not be called.
  void Mutate(std::vector<ByteArray> &inputs) override { CHECK(false); }

  // Allows to call a protected member function in a test.
  bool MutateViaExternalBinary(std::string_view binary,
                               const std::vector<ByteArray> &inputs,
                               std::vector<ByteArray> &mutants) {
    return CentipedeCallbacks::MutateViaExternalBinary(binary, inputs, mutants);
  }
};

TEST(Centipede, MutateViaExternalBinary) {
  // This binary contains a test-friendly custom mutator.
  auto binary_with_custom_mutator = devtools_build::GetDataDependencyFilepath(
      "google3/third_party/centipede/testing/test_fuzz_target_sancov");
  // This binary does not contain a custom mutator.
  auto binary_without_custom_mutator =
      devtools_build::GetDataDependencyFilepath(
          "google3/third_party/centipede/testing/abort_fuzz_target_sancov");
  ...
  EXPECT_FALSE(callbacks.MutateViaExternalBinary(binary_without_custom_mutator,
                                                 inputs, mutants));
...
class MergeMock : public CentipedeCallbacks {
 public:
  MergeMock(const Environment &env) : CentipedeCallbacks(env) {}
  ...
  void Mutate(std::vector<ByteArray> &inputs) override {
    for (auto &input : inputs) {
      ...
      input[0] = number_of_mutations_++;  // first mutation is {0}.
    }
  }
...
class FunctionFilterMock : public CentipedeCallbacks {
 public:
  FunctionFilterMock(const Environment &env) : CentipedeCallbacks(env) {}
  ...
  void Mutate(std::vector<ByteArray> &inputs) override {
    for (auto &input : inputs) {
      ...
      input = GetMutant(++number_of_mutations_);
    }
  }
  ...
  static ByteArray GetMutant(size_t idx) {
    ...
    return ByteArray(mutant, mutant + strlen(mutant));
  }
...
static std::vector<ByteArray> RunWithFunctionFilter(
  ...
  env.num_runs = 100;
  env.batch_size = 10;
  env.binary = devtools_build::GetDataDependencyFilepath(
      "google3/third_party/centipede/testing/test_fuzz_target_sancov");
  env.coverage_binary = env.binary;
  // Must symbolize in order for the filter to work.
  env.llvm_symbolizer_path = devtools_build::GetDataDependencyFilepath(
...
class ExtraBinariesMock : public CentipedeCallbacks {
 public:
  ExtraBinariesMock(const Environment &env) : CentipedeCallbacks(env) {}
  ...
  // Sets the inputs to different 1-byte values.
  void Mutate(std::vector<ByteArray> &inputs) override {
    for (auto &input : inputs) {
      input.resize(1);
      input[0] = ++number_of_mutations_;
    }
  }
