...
class CentipedeCallbacks {
 public:
  ...
  // Returns non-zero number if PC table was not populated and
  // `env_.require_pc_table` is set. Returns 0 otherwise.
  virtual int PopulateSymbolAndPcTables(SymbolTable &symbols,
                                        Coverage::PCTable &pc_table) {
    // TODO(sarahjkim) Turn this into a pure virtual method once default
    // implementation is added to CentipedeDefaultCallbacks.
    return EXIT_SUCCESS;
  }
