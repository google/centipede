...
class CentipedeCallbacks {
 public:
  ...
  // Returns false if PC table was not populated and `env_.require_pc_table` is
  // set. Returns true otherwise.
  virtual bool PopulateSymbolAndPcTables(SymbolTable &symbols,
                                         Coverage::PCTable &pc_table) = 0;
