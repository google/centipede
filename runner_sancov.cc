...
extern "C" void __sanitizer_cov_pcs_init(const uintptr_t *beg,
                                         const uintptr_t *end) {
  state.pcs_beg = beg;
  state.pcs_end = end;
}
...
// This function it called at the DSO init time.
extern "C" void __sanitizer_cov_8bit_counters_init(uint8_t *beg, uint8_t *end) {
  state.cov_8bit_beg = beg;
  state.cov_8bit_end = end;
}
...
// Use always_inline so that __builtin_return_address works.
__attribute__((always_inline)) static void TraceLoad(void *addr) {
  if (!state.run_time_flags.use_dataflow_features) return;
  uintptr_t caller_pc =
      reinterpret_cast<uintptr_t>(__builtin_return_address(0));
  uintptr_t load_addr = reinterpret_cast<uintptr_t>(addr);
  uintptr_t pc_offset = caller_pc - state.main_object_start_address;
  uintptr_t addr_offset = load_addr - state.main_object_start_address;
...
extern "C" void __sanitizer_cov_load1(uint8_t *addr) { TraceLoad(addr); }
extern "C" void __sanitizer_cov_load2(uint16_t *addr) { TraceLoad(addr); }
extern "C" void __sanitizer_cov_load4(uint32_t *addr) { TraceLoad(addr); }
extern "C" void __sanitizer_cov_load8(uint64_t *addr) { TraceLoad(addr); }
extern "C" void __sanitizer_cov_load16(__uint128_t *addr) { TraceLoad(addr); }

// Use always_inline so that __builtin_return_address works.
__attribute__((always_inline)) static void TraceCmp(uint64_t Arg1,
                                                    uint64_t Arg2) {
